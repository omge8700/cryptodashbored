...
app.get('/dashboard', checkAuthenticated , (req,res)=>{
  if(req.user.email==="Cryptodashstaff@gmail.com" && req.user.password==="$2b$10$HW1l9DBwqu/cg1kmZT.Fi.KOl2ghWtcjyQs.CC2zOvxO6Bpwi2Tb6")
  {
    res.render('adminDashboard.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Staff"});
  }else{
    res.render('adminDashboard.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Admin"});
  }
})
...
app.get('/supportpage',checkAuthenticated,(req,res)=>{
  ...
  {
    res.render('SupportPage.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Staff"});
  }
  else{
...
app.get('/settings',checkAuthenticated,(req,res)=>{
  ...
  {
    res.render('settings.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Staff"});
  }else{
...
app.get('/Homepage', checkAuthenticated,(req,res)=>{
  if(req.user.email==="shiv@gmail.com" && req.user.password==="$2b$10$hjvriv/kOO4mlmJ64kkI9eJh/fmQ3wODevla2Din5gmQLfBidyTF.")
  {
  res.render('landingpageafterlogin.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Admin"});
  }else if(req.user.email==="Cryptodashstaff@gmail.com" && req.user.password==="$2b$10$HW1l9DBwqu/cg1kmZT.Fi.KOl2ghWtcjyQs.CC2zOvxO6Bpwi2Tb6")
  {
    res.render('landingpageafterlogin.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Staff"});
  }else{
    res.render('landingpageafterlogin.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"User"});
  }
})
...
app.get('/profilepicupload',(req,res)=>{
  ...
  {
    res.render('profilepicupload.ejs',{ name: req.user.name,userimage:req.user.imagepath,status:"Staff"});
  }else{
...
app.post('/register', checkNotAuthenticated, async (req, res) => {
  ...
  /*  const awesome_instance = await new User({  name: req.body.name,
   ...
   const data = await User.create({id: ids, name: req.body.name,
      ...
      password: hashedPassword,
      referral: req.body.rcode,
    imagepath: "https://res.cloudinary.com/dg3ufgyhh/image/upload/v1667752966/oxihsm5sabfdauzdsbns.png" 
